...
LASTID1=$(sbatch --export=ALL,CONFIG=$CONFIG,OPTUNA=$OPTUNA,WANDB=$WANDB crowd.sbatch)
LASTID1=$(echo $LASTID1 | awk 'NF{ print $NF }')

echo "Launched $LASTID1"
LASTID2=$(sbatch --export=ALL,CONFIG=$CONFIG,OPTUNA=$OPTUNA,WANDB=$WANDB crowd.sbatch)
...
do
    ...
    LASTID2=$(sbatch --dependency=afterok:$LASTID2 --export=ALL,CONFIG=$CONFIG,OPTUNA=$OPTUNA,WANDB=$WANDB crowd.sbatch)
    LASTID2=$(echo $LASTID2 | awk 'NF{ print $NF }')
